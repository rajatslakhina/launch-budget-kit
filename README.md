...
- ✅ **`swift test` passes 117/117.** 23 of those are regression tests, each pinned to a specific defect that independent review rounds found *after* the original 94-test suite already passed. Coverage is weighted toward the degenerate inputs rather than the happy path: empty graphs, self-edges, 4-node cycles, a 2,000-deep dependency chain, out-of-range and `Int.min`/`Int.max` indices, `NaN`/`±∞`/negative durations, zero sample intervals, empty traces, fully-unsymbolicated traces, re-entrant stacks, merge-denial cascades, calibration refusals, and determinism (asserted by running the same computation 20× and comparing).
...
- ❌ **Never compiled by Xcode, and never run.** The environment that produced this had no macOS toolchain — no `xcodebuild`, no iOS SDK — and it was an unattended scheduled job, which cannot be granted permission to drive the screen. So the demo app was **not launched on a Simulator, not interacted with, and not screenshotted. There are no screenshots in either repository, because there is no honest screenshot to put in one.**
- ⚠️ **Therefore the SwiftUI layer is unverified.** `LaunchBudgetKitUI` compiling on Linux means only that `#if canImport(SwiftUI)` compiled it out — the green Linux build says nothing whatsoever about that code. It was reviewed line by line (no force-unwraps, no unchecked collection access) and the macOS/iOS-SDK CI jobs in `.github/workflows/ci.yml` exist to close this gap, but **they have not run yet** — they were added in the same change. The one defect of that class found so far (a `navigationBarTitleDisplayMode` call unavailable on macOS) was caught by review, not by a build.

If you open this in Xcode and something doesn't compile, that's a real gap in the verification story above and not a surprise — please open an issue.
